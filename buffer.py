...
class TrajectoryReplayBuffer:
    ...
    def add_transition(self, z_t: torch.Tensor, a_t: torch.Tensor, r_t: torch.Tensor, z_tp1: torch.Tensor):
        self.data.append((
            z_t.detach().to(self.device).float(),
            a_t.detach().to(self.device).float(),
            r_t.detach().to(self.device).float(),
            z_tp1.detach().to(self.device).float(),
        ))
    ...
    def sample(self, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        return dict(
            ...
            z_tp1=torch.stack(z_tp1, 0).to(self.device),
        )